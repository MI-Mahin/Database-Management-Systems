--1(a)
DECLARE 
  v_first_name VARCHAR2(50) := 'Mahmudul';
BEGIN
  DBMS_OUTPUT.PUT_LINE('My first name is: ' || v_first_name);
END;
/
...
BEGIN
  v_student_id := '210042140';

  DBMS_OUTPUT.PUT_LINE('Length of student ID ' || v_student_id || ' is: ' || LENGTH(v_student_id));
END;
/
...
--2(b)
CREATE OR REPLACE PROCEDURE print_students IS
BEGIN
  FOR course_rec IN (SELECT * FROM course) LOOP
    FOR student_rec IN (
      SELECT s.name
      FROM student s
      LEFT JOIN takes t ON s.ID = t.ID
      LEFT JOIN prereq p ON t.course_id = p.course_id
      WHERE p.prereq_id IS NOT NULL AND p.prereq_id = course_rec.course_id
    ) LOOP
      DBMS_OUTPUT.PUT_LINE('Course Title: ' || course_rec.title);
      DBMS_OUTPUT.PUT_LINE('Student Eligible: ' || NVL(student_rec.name, 'None')); -- Display 'None' if no eligible student
